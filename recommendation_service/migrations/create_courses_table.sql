...
-- Policy: Institutions can view their own courses
CREATE POLICY "Institutions can view their own courses"
    ON courses FOR SELECT
    USING (auth.uid() = institution_id);

-- Policy: Institutions can insert their own courses
CREATE POLICY "Institutions can insert their own courses"
    ON courses FOR INSERT
    WITH CHECK (auth.uid() = institution_id);

-- Policy: Institutions can update their own courses
CREATE POLICY "Institutions can update their own courses"
    ON courses FOR UPDATE
    USING (auth.uid() = institution_id)
    WITH CHECK (auth.uid() = institution_id);

-- Policy: Institutions can delete their own courses
CREATE POLICY "Institutions can delete their own courses"
